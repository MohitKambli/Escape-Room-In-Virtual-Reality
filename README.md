...
<ul>
  <li><p>Picking up key from the room of a specified color: </p>In this puzzle, the student has to search for a key of specified color in order to hop into the next room. This puzzle covers a basic “select” query that is present in database concepts.</li>
  <li><p>Popping the blue balls, and pushing the red balls: </p>The concept involved in this example is stack, which has two operations of push and pop. The written clue or puzzle inside the room will give a hint about it. Students need to crack the hint and should take out (pop) blue balls of the mug and should put (push) red balls into it.</li>
  <li><p>Selecting the correct door that wraps the IP: </p>This puzzle consists of a specific IP that belongs to a particular class. Students have to determine the correct class that IP belongs to and go for that door. If the answer is incorrect, then he has to face the circumstances, that is, either get a massive hit by burning in fire, or by falling down.</li>
</ul>
